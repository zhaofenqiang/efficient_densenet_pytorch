...
imgTensorNormalize = train_transforms(img)
imgTensor = imgTensorNormalize
for t, m, s in zip(imgTensorNormalize, mean, stdv):
    t.mul_(s).add_(m)
plt.imshow(imgTensorNormalize.numpy().transpose((1, 2, 0)))
